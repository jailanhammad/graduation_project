...
const Gallery = () => {
    ...
    const fetchGallery = async () => {
        ...
        if (!error && data && data.length > 0) {
            setDbImages(data.map(item => item.image_url));
        }
    };
    ...
    const staticImages = [range1, range2, range3, range4, range5, range6, range7, range8, range9, range10];
    const displayImages = dbImages.length > 0 ? dbImages : staticImages;
